...
## IMPORTANT NOTE: USE ONLY IN BCV002 BECAUSE THE DATASET AND MODEL WEIGHTS ARE LOCATED THERE
...
def main():
    ...
    parser.add_argument('-mode', type=str, default=None, help="test or demo") 
    parser.add_argument('-img', type=str, default=None, help="path to LR image") 
    ...
    args.scale = 2
    args.model_path = "/media/SSD0/idchacon/Visión/Proyecto/KAIR/superresolution/swinir_sr_classical_patch48_heads/models/10000_G.pth" if args.use_heads else "/media/SSD0/idchacon/Visión/Proyecto/KAIR/superresolution/swinir_sr_classical_patch48_x2_fineTune/models/30000_G.pth"
    args.folder_lq = "/media/SSD0/jsurrea/SR/test/LR"
    args.folder_gt = "/media/SSD0/jsurrea/SR/test/HR"
    ...
    if args.mode == "test":
        ...
        # Get path
        path = args.img
        (imgdir, basename) = os.path.split(path)
        (imgname, imgext) = os.path.splitext(basename)
        ...
        if output.ndim == 3:
            output = np.transpose(output[[2, 1, 0], :, :], (1, 2, 0))  # CHW-RGB to HCW-BGR
        output = (output * 255.0).round().astype(np.uint8)  # float32 to uint8
        cv2.imwrite(f'{imgdir}/{imgname + "_SR.png"}', output)
        print("Saved SR image on path:", f'{imgdir}/{imgname + "_SR.png"}')
...
def define_model(args):
    ...
    if args.task == 'classical_sr':
        num_heads = 4 if args.use_heads else 6
        model = net(upscale=args.scale, in_chans=3, img_size=args.training_patch_size, window_size=8,
                    img_range=1., depths=[6, 6, 6, 6, 6, 6], embed_dim=180, num_heads=[num_heads]*6,
